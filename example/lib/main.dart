...
class _AppState extends State<App> {
    ...
    if (_isAndroidEmulator || _isIosSimulator)
      ...
      content = Column(children: [
        ...
        Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
          ...
          RaisedButton(child: Text('Stop'), onPressed: () => _audioVideoStop())
        ]),
  ...
  void _handleOnVideoTileAdded(dynamic arguments) async {
    ...
    setState(() {
      attendee.setVideoView(ChimeDefaultVideoRenderView(
          onPlatformViewCreated: (int viewId) async {
        attendee.setViewId(viewId);
        print(
            'ChimeDefaultVideoRenderView created. TileId=${attendee.tileId}, ViewId=${attendee.viewId}, VideoView=${attendee.videoView} => binding');
        await Chime.bindVideoView(attendee.viewId, attendee.tileId);
        print(
            'ChimeDefaultVideoRenderView created. TileId=${attendee.tileId}, ViewId=${attendee.viewId}, VideoView=${attendee.videoView} => bound');
      }));
    });
  }
